...
UNIT_NAME="${BESTCRM_EMAIL_BACKFILL_UNIT:-bestcrm-email-backfill.service}"
...
cat > "$TMP_UNIT" <<UNIT
[Unit]
Description=BESTCRM bounded historical email backfill
...
ExecStart=/usr/bin/node scripts/poll-email-inquiries.mjs --backfill-continuous
Restart=on-failure
RestartSec=300
User=${SERVICE_USER}
Group=${SERVICE_GROUP}
...
echo "The unit was not enabled or started. Validate configuration before activation."
