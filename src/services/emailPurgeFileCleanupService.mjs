...
function safeErrorDetail(error) {
  ...
  return 'Stored file could not be removed';
}
...
async function verifyAndRemoveFile(uploadDir, job) {
  ...
  if (!fileStat.isFile()) throw cleanupError('not_a_file', 'Stored path is not a file');
  if (Number(fileStat.size) !== Number(job.expectedSize)) {
...
export function supportsEmailPurgeFileCleanup(repository) {
  return Boolean(
    repository
    && typeof repository.claimEmailPurgeFileJobs === 'function'
    && typeof repository.completeEmailPurgeFileJob === 'function'
    && typeof repository.failEmailPurgeFileJob === 'function'
  );
}

export async function processEmailPurgeFileJobs(dependencies, options = {}) {
  const repository = dependencies?.emailArchiveRepository;
  if (!supportsEmailPurgeFileCleanup(repository)) {
    throw cleanupError('cleanup_queue_unavailable', 'Email purge file cleanup queue is unavailable');
  }
  ...
  for (let batch = 0; batch < maxBatches; batch += 1) {
    ...
    for (const job of jobs) {
      try {
        ...
        await repository.failEmailPurgeFileJob({
          jobId: job.id,
          workerId,
  ...
  if (typeof repository.countEmailPurgeFileJobs === 'function') {
    result.pending = await repository.countEmailPurgeFileJobs({ purgeAuditIds });
  }
  return result;
}

export function startEmailPurgeFileCleanupLoop(dependencies, options = {}) {
  ...
  const runNow = async () => {
    ...
    running = true;
    try {
      const result = await processEmailPurgeFileJobs(dependencies, options);
      if (result.failed > 0 || result.pending > 0) {
        logger.warn?.(JSON.stringify({
          event: 'email_purge_file_cleanup_deferred',
      ...
      logger.error?.(JSON.stringify({
        event: 'email_purge_file_cleanup_failed',
