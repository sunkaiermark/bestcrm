...
test('inquiry purge worker pauses before unlink when backup write maintenance becomes active', async () => {
  const uploadDir = await mkdtemp(path.join(tmpdir(), 'bestcrm-inquiry-purge-maintenance-'));
  const storedPath = 'email-inquiries/maintenance.pdf';
  const content = 'preserve while backup is running';
  await mkdir(path.dirname(path.join(uploadDir, storedPath)), { recursive: true });
  await writeFile(path.join(uploadDir, storedPath), content);
  const repository = createQueueRepository([{
    id: 6, purgeAuditId: 95, storedPath,
    expectedSize: Buffer.byteLength(content), expectedSha256: sha256(content)
  }]);
